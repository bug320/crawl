...
def saveIntoMySQL(fname,db=None):
    ...
    pass
def saveIntoHbase(url):
    #db = MySQL.MySQL(host="localhost", usr="root", passwd="root", dbase='bug320') if db == None else db
    #hdb=hbase_mar.HbaseClient("localhost", 9090)
    #  create 'subinfo','id','url','info';
    #3links = openCSV(fname)
    i=0
    #for link in links:
    if True:
        i += 1
        ...
        if save == False:
            ...
            tmp=getInnerPageURLs(soup)
            url = "%s%s" % (mainHTTP,tmp[0][0])
            print url
            html = download(link, headers)
            if html ==None:
                save =False
                #hdb.put("subinfo",{"id":i},{"url":link},{"info":""})
            soup=BeautifulSoup(html,htmlparser)
            if soup ==None:
                save = False
                #hdb.put("subinfo", {"id": i}, {"url": link[1]}, {"info": ""})
            if save:
                saveTxt=""
                for string in soup.stripped_strings:  #<a>innerHTML</a>
                    dammit = UnicodeDammit(string)
                    # 调试输出
                    # print dammit.unicode_markup
                    saveTxt=saveTxt+"%s\n" % dammit.unicode_markup.encode('utf-8')
                    hdb.put("subinfo", {"page:id": i}, {"page:url": link[1]}, {"page:info": saveTxt})
    pass
	...
	for link in links:
		...
		if save == False:
			...
			if not db.selectSQL("subpage", sql):
				# print bool(db.selectSQL("subpage",sql))
				# db.execSQL("subpage","set names utf8")
				db.insert("subpage", url=link[1], title=attr[0], gongsi=attr[1], person=attr[2], time=attr[3],
						  user=attr[4], flag=str(save))
				#hdb.put()
			else:
				##print  "OK"
				sql = "url = '%s'" % (link[1])
				...
				if bool(db.selectSQL("subpage", sql)[0]['flag']) == False:
					sql = "url = '%s'" % (link[1])
					# title = attr[0], gongsi = attr[1], person = attr[2], time = attr[3], user = attr[4], flag = str(save)
					db.update(title=attr[0], gongsi=attr[1], person=attr[2], time=attr[3], user=attr[4], flag=str(save),
							  where=sql)
				else:
					print "Ok"
